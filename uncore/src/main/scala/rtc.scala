...
class RTC(csr_MTIME: Int)(implicit p: Parameters) extends HtifModule
  ...
  when(!(!rtc_tick || send_acked.reduce(_ &&_))){
    printf("[hgmoon-dbg]\trtc_tick:\t%x\n",rtc_tick)
    for (i <- 0 until nCores){
      printf("[hgmoon-dbg]\tsend_acked(%x):\t%x\n",UInt(i),send_acked(i))

    }
    printf("[hgmoon-dbg]\tsend_acked.reduce(_ &&_):\t%x\n",send_acked.reduce(_&&_))
  }
