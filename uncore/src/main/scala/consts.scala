...
trait MemoryOpConstants {
  ...
  val M_XRDUM   = UInt("b10100"); // int load and unmask
  val M_XWRM    = UInt("b10101"); // int store and mask

  def isAMO(cmd: UInt) = cmd(3) || cmd === M_XA_SWAP
  def isPrefetch(cmd: UInt) = cmd === M_PFR || cmd === M_PFW
  def isRead(cmd: UInt) = cmd === M_XRD || cmd === M_XRDUM || cmd === M_XLR || cmd === M_XSC || isAMO(cmd)
  def isWrite(cmd: UInt) = cmd === M_XWR || cmd === M_XWRM || cmd === M_XSC || isAMO(cmd)
  def isWriteIntent(cmd: UInt) = isWrite(cmd) || cmd === M_PFW || cmd === M_XLR
  def needsMask(cmd: UInt) = cmd === M_XRDUM || cmd === M_XWRM
}
