...
class Frontend(implicit p: Parameters) extends CoreModule()(p) with HasL1CacheParameters {
  ...
  require(fetchWidth * coreInstBytes != rowBytes)
  io.dr_next_insn.bits.data := 
    s2_resp_data >> (s1_pc(log2Up(rowBytes)-1,log2Up(fetchWidth*coreInstBytes)) << log2Up(fetchWidth*coreInstBits))
 
  io.dr_next_insn.valid :=  io.cpu.resp.valid 
